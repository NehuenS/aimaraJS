...
// p_connectingLines: True or false, indicates if the nodes are connected by dotted lines;
// p_rightSide: True or false, indicating if the tree should be on the right side.
function createTree(p_div,p_backColor,p_contextMenu,p_connectingLines,p_rightSide) {
	var tree = {
		name: 'tree',
		div: p_div,
		ulElement: null,
		...
		drawTree: function() {
			...
			if(this.connected){
				div_tree.className += " linked"
			}
			if(this.flipped){
				div_tree.className += " mirrored"
			}
